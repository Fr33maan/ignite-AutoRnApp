...
test('does not have a linting script', () => {
...
describe('generators', () => {
  ...
  test('generates a component', async () => {
    execa(IGNITE, ['g', 'app', 'test'], { preferLocal: false }).stdout.pipe(process.stdout)
  ...
  test('generate listview of type row works', async () => {
    // await execa(IGNITE, ['ignite-ir-boilerplate-plus', 'screen-plus', 'Test', ''], { preferLocal: false })
    // expect(jetpack.exists('App/Containers/Test.js')).toBe('file')
    // expect(jetpack.exists('App/Containers/Styles/TestStyle.js')).toBe('file')
