...
export default class ConfigBuilder {
  ...
  createReduxStates (hoc,) {
    ...
    if ('model' in hoc) {
      ...
      states = R.unnest(objectToArray(hoc.subs, (subHoc, subHocName) => [`doing${capitalize(subHocName)}`, `done${capitalize(subHocName)}`]))
    ...
    if ('actions' in hoc) {
      states = states.concat(R.unnest(objectToArray(hoc.actions, (action, actionName) => [`doing${capitalize(actionName)}`, `done${capitalize(actionName)}`])))
    }

    // Add hoc props to state
    if ('props' in hoc) {
  ...
  } else if(hoc.level !== 1 && !hoc.includeInStack) {
      actions = [{
        name: hoc.name,
        args: hoc.props || [],
        res: hoc.response || [],
  ...
  createTemplateImports (hoc, props, jobType) {
    ...
    if(jobType === 'hoc') {
      ...
      .concat((() => {
          if(hoc.level === 0) {
            // return [`import { ${hoc.actions.map(action => action.name + 'Request').join(', \n\t')} } from '${hoc.reduxRootDir}/rdx.${hoc.Name}'`]
            return [`import * as ${hoc.name}Actions from '${hoc.reduxRootDir}/rdx.${hoc.Name}'`]
          } else if(hoc.level === 2) {
            return [`import * as ${hoc.Name}Actions from '${hoc.reduxRootDir}/rdx.${hoc.Name}'`]

            // Import additional actions from hoc Redux file
          } else if(hoc.actions.length > 0) {
            return [`import * as ${hoc.name}Actions from '${hoc.reduxRootDir}/rdx.${hoc.Name}'`]
            // return [`import { ${hoc.actions.map(action => action.name + 'Request').join(', \n\t')} } from '${hoc.reduxRootDir}/rdx.${hoc.Name}'`]
          } else {
