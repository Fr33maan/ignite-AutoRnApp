...
export default class ConfigBuilder {
  ...
  buildConfig () {
    ...
    const process = (hoc, hocName, level, pathModifier) => (function * () {
      ...
      yield confBuilder.transformModel(hoc)
    ...
    this.modalsInitialState = JSON.stringify(R.mergeAll(this.modals.map(modal => ({[`show${modal.Name}Modal`]: false, [`${modal.name}ModalParams`]: {}}))), null, '\t')
  }
  ...
  setRCompAttributes (hoc, hocName, level, pathModifier) {
    ...
    hoc.subsNames = objectToArray(hoc.subs, (sub, subName) => ({
      ...
      NAME: camelToScreamingSnake(subName)
    }))
  }
  ...
  createFileJobs (hoc) {
    ...
    const forcedType = hoc.level === 1 && !hoc.excludeFromStack ? 'submodal' : hocType
  ...
  createReduxStates (hoc) {
    ...
    if ('model' in hoc) {
      ...
      states = states.concat([`doingUpdate${hoc.Name}`  , `doneUpdate${hoc.Name}`])
  ...
  createReduxActions (hoc, level) {
    ...
    if ('actions' in hoc) {
      ...
      actions = [...actions, ...singleActions.map(action => ({
        ...
        NAME: camelToScreamingSnake(action.name)
      }))]
    }
  ...
  configureNavs (hoc) {
    if ('navs' in hoc) {
      hoc.navs = hoc.navs.map(nav => {
        ...
        let param = null
        let type = 'nav'
        // Look for a nav parameter
        if(new RegExp(/:/).test(nav)) {
          name = nav.split(':')[0]
          param = nav.split(':')[1]
        }

        // Define if nav is an outside modal or in the current stack
  ...
  transformModel (hoc) {  
    if(hoc.model) {
      ...
      for (let propName in hoc.model) {
        const prop = hoc.model[propName]
        newModel[propName] = {}
        newModel[propName].prop = prop
        newModel[propName].inputType = getInputType(prop)
        newModel[propName].defaultValue = getDefaultValue(prop)
        newModel[propName].name = propName
        newModel[propName].Name = capitalize(propName)
        newModel[propName].NAME = propName.toUpperCase()
      }
  ...
  createTemplateMiscProps (hoc, props) {
    ...
    props.isEditable       = hoc.isEditable
    props.isForm           = hoc.isForm
    props.navs             = hoc.navs
    props.excludeFromStack = hoc.excludeFromStack
    ...
    props.model            = hoc.model
    props.modelProps       = objectToArray(hoc.model, (obj, objName) => objName)
  }
}
